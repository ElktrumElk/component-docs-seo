  ...
  return (
    <div>
      ...
      <p className="text-text-secondary text-sm sm:text-base leading-relaxed max-w-xl mb-8">
        ...
        <span className="highlight-amber">zero config</span> — import any
        component or hook and start building immediately.
      </p>
      ...
      <HighlightedCode filename="terminal">
...
<span class="keyword">pnpm</span> add <span class="string">elk-components</span>`}
      ...
      <div className="mt-8 p-5 rounded-xl border border-border bg-surface-card/50">
        ...
        <p className="text-text-secondary text-sm leading-relaxed">
          elk-components requires{" "}
          <code className="text-warm-accent font-mono text-xs">react</code> and{" "}
          <code className="text-warm-accent font-mono text-xs">react-dom</code>{" "}
          as peer dependencies. For Motion mode animations, it also uses{" "}
          <code className="text-cool-accent font-mono text-xs">
            motion/react
          </code>{" "}
          which is bundled as a dependency.
        </p>
      </div>
      ...
      <div className="space-y-5">
        <div>
          <p className="text-text-secondary text-sm mb-3">
            <span className="highlight-green">Named imports</span> — import
            individual components directly. Tree-shaking removes any unused
            code.
          </p>
          <HighlightedCode filename="App.tsx">
...
<span class="keyword">import</span> { Animation, Transition } <span class="keyword">from</span> <span class="string">"elk-components"</span>;`}
        ...
        <div>
          <p className="text-text-secondary text-sm mb-3">
            <span className="highlight-amber">Hooks</span> — import from the
            hooks subpath. All hooks are tree-shakeable.
          </p>
          <HighlightedCode filename="hooks.ts">
{`<span class="keyword">import</span> {
  ...
  useStableCallback
        ...
        <div>
          <p className="text-text-secondary text-sm mb-3">
            <span className="highlight-cyan">Scaffold a new project</span> — use
            the CLI to generate a starter component file with all required
            imports.
          </p>
          <HighlightedCode filename="terminal">
